...
void os_perso_set_seed ( unsigned int algorithm, unsigned char * seed, unsigned int length ) 
{
  unsigned int ret;
  unsigned int parameters [2+3];
  parameters[0] = (unsigned int)SYSCALL_os_perso_set_seed_ID_IN;
  parameters[1] = (unsigned int)G_try_last_open_context->jmp_buf;
  parameters[2] = (unsigned int)algorithm;
  parameters[3] = (unsigned int)seed;
...
unsigned int os_registry_get_icon ( unsigned int appidx, unsigned int offset, unsigned char * buffer, unsigned int maxlength ) 
{
  unsigned int ret;
  unsigned int parameters [2+4];
  parameters[0] = (unsigned int)SYSCALL_os_registry_get_icon_ID_IN;
  parameters[1] = (unsigned int)G_try_last_open_context->jmp_buf;
  parameters[2] = (unsigned int)appidx;
  parameters[3] = (unsigned int)offset;
  ...
  parameters[5] = (unsigned int)maxlength;
                              ...
                              asm volatile("svc #1");
                              asm volatile("mov %0, r0":"=r"(ret));
                                if (parameters[0] != SYSCALL_os_registry_get_icon_ID_OUT)
  {
    THROW(EXCEPTION_SECURITY);
  }
  return (unsigned int)ret;
